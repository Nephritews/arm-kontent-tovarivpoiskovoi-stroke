...
### Пример
Был создан блок с товарами в поиской строке. Сначала будем его проверять на дев версии сайта. Для этого будет указан дев сервер и даты начала и конца активности этого блока. Дата нужна для того, чтобы можно было указать, в каком временном диапазоне этот блок используется, (например, в следующем месяце).
